...
export class MetricaAvaliacaoComponent implements OnChanges, OnInit {
  ...
  /** Métricas escalares (exclui matrizes de confusão) para a tabela. */
  ...
  async baixarRelatorioAluno(): Promise<void> {
    ...
    for (const grupo of this.getVisualizacoesPorTipo()) {
      for (const item of grupo.itens) {
        const info = this.montarGraficoItemInfo(item.visualizacao as any);
        graficos.push({
          titulo: grupo.titulo,
          modelo: item.modelo,
          dataUrl: this.getImagemVisualizacao(item.visualizacao),
          discussao: info.resumo_basico || info.descricao || '',
          dicas: info.dicas || [],
        });
      }
    }

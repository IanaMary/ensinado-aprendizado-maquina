...
export class RelatorioPdfService {
  ...
  async gerar(input: RelatorioPdfInput): Promise<void> {
    ...
    //
    // Sobre `•` e `—` no texto deste relatório: eles RENDERIZAM. O jsPDF declara as fontes core
    // com `/Encoding /WinAnsiEncoding`, onde `•` é 0x95 e `—` é 0x97 — verificado nos bytes do
    // PDF gerado. Houve um relato de que saíam como espaço; era erro da instrumentação que o
    // mediu (`String.fromCharCode(0x95)` dá um caractere de controle invisível). Não troque por
    // ASCII sem antes conferir o `/Encoding` do PDF de verdade.
